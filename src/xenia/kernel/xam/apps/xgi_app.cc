...
X_RESULT XgiApp::DispatchMessageSync(uint32_t message, uint32_t buffer_ptr,
  ...
  switch (message) {
    ...
    case 0x000B0008: {
      ...
      char title_id[9] = {0};
      std::snprintf(title_id, xe::countof(title_id), "%08X",
                    module->title_id());
      module->GetSection(title_id, &resource_data, &resource_size);
      xe::kernel::util::XdbfWrapper wrapper(
          module->memory()->TranslateVirtual(resource_data), resource_size);

      std::vector<xe::kernel::util::XbdfAchievement> entries;

      auto achievementCount = wrapper.GetAchievements(
          xe::kernel::util::XdbfLocale::kEnglish, &entries);
      uint32_t i;
      ...
      auto img_info = wrapper.GetEntry(xe::kernel::util::XdbfSection::kImage,
                                       achievement.image_id);
      if (img_info) {
      ...
      XELOGI("Achievement gained! %s (id: %u, gamerscore: %u, description: %s)",
             achievement.label.c_str(), achievement.id, achievement.gamerscore,
